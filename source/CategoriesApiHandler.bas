...
Private Sub GetCategories
	...
	Dim OrderKeys As List = Array("id", "category_name", "created_date", "modified_date")
	Dim CloneList As MinimaList = Main.CategoriesList.Clone
...
Private Sub GetCategoryById (id As Int)
	Log($"${Request.Method}: ${Request.RequestURI}"$)
	Dim CloneList As MinimaList = Main.CategoriesList.Clone
	Dim M1 As Map = CloneList.Find(id)
	Dim OrderKeys As List = Array("id", "category_name", "created_date", "modified_date")
	If HRM.OrderedKeys Then M1.Put("__order", OrderKeys)
	If M1.Size > 0 Then
		HRM.ResponseCode = 200
...
Private Sub UpdateCategoryById (id As Int)
	...
	Dim M2 As Map = Main.CategoriesList.CopyObject(M1)
	Dim OrderKeys As List = Array("id", "category_name", "created_date", "modified_date")
